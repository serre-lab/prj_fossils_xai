...
def pi(img, mask):
    ...
    img2_resize = tf.image.resize(img2, (SIZE, SIZE), method="bicubic", antialias=True)
    img_pad = tf.image.resize_with_pad(
        img, SIZE, SIZE, method="bicubic", antialias=True
    )
    img2_pad = tf.image.resize_with_pad(
        img2, SIZE, SIZE, method="bicubic", antialias=True
    )
    ...
    if anchor_xmax - anchor_xmin > 50 and anchor_ymax - anchor_ymin > 50:
        ...
        img2_anchor_1 = resize(img2[anchor_xmin:anchor_xmax, anchor_ymin:anchor_ymax])
        ...
        img2_anchor_2 = resize(img2_anchor_2)
        img_anchor_2 = resize(img_anchor_2)
    else:
        img_anchor_1 = img_resize
        img_anchor_2 = img_pad
        img2_anchor_1 = img2_resize
        img2_anchor_2 = img2_pad
    ...
    img_max_zoom1 = img[
        tf.math.maximum(anchor_max_x - SIZE // 6, 0) : tf.math.minimum(
            anchor_max_x + SIZE // 6, w
        ),
        tf.math.maximum(anchor_max_y - SIZE // 6, 0) : tf.math.minimum(
            anchor_max_y + SIZE // 6, h
        ),
    ]
    ...
    img_max_zoom2 = img[
        tf.math.maximum(anchor_max_x - SIZE // 3, 0) : tf.math.minimum(
            anchor_max_x + SIZE // 3, w
        ),
        tf.math.maximum(anchor_max_y - SIZE // 3, 0) : tf.math.minimum(
            anchor_max_y + SIZE // 3, h
        ),
    ]
    ...
    return tf.cast(
        ...
        tf.float32,
    ), tf.cast([img2_resize, img2_anchor_1, img2_anchor_2], tf.float32)
...
def segmentation_sam(batch_input, batch_labels, batch_domain, batch_size):
    ...
    for x, y, d in zip(X, batch_labels, batch_domain):
        ...
        sample, un_mask_sample = pi(x, mask)
        reps = sample.shape[0]
        label = tf.repeat([tf.math.argmax(y)], reps, axis=0)
        samples.append(sample)
        un_mask_samples.append(un_mask_sample)
        labels.append(label)
        domains.append(tf.repeat([d], reps, axis=0))

    return (
        tf.stack(un_mask_samples, axis=0),
...
def evaluate(model, test_ds, batch_size=10, prob_augmentation=0.9, val_ds=None):
    ...
    for b, (batch_inputs, batch_labels, batch_domain) in enumerate(test_ds):
        unmask_images, images, labels, domains = segmentation_sam(
            batch_inputs * 255, batch_labels, batch_domain, b
        )
        print(images.shape)
        labels = labels[:, 0]
        class_0images = images[labels == 0].numpy()
        n_images = len(class_0images)
        for i in range(n_images):
            img = cv2.cvtColor(class_0images[i][0], cv2.COLOR_RGB2BGR)
            cv2.imwrite(f"maskedimages/anacardiaceae/{total_count}.png", img)
            total_count += 1
        continue
        ...
        unmask_input_images = tf.reshape(unmask_images, (-1, 384, 384, 3)) / 255
        labels = labels.numpy()
        ...
        final_images = unmask_input_images[labels == final_predictions]
        ...
        if len(final_images) > 0:
            ...
            activations = tf.gather(
                activations, tf.where(percentage_black_pixels < 95)
            )[:, 0, :]
            patch_labels = tf.gather(
                patch_labels, tf.where(percentage_black_pixels < 95)
            )[:, 0]
            ...
            if b > 0:
                activations_and_patches = np.load(
                    "./activations/activations_patches_resnet5.npz"
                )
                ...
                image_activations = np.concatenate(
                    [prev_image_activations, image_activations]
                )
            ...
            if b % 12 == 0 and b != 0:
                ...
                start = len(all_importances)
                for i in range(start, n_image_activations, 4000):
                ...
                all_importances = all_importances.tolist()
...
if __name__ == "__main__":
    ...
    save_crops = "./crops/fossils_leaves_crops/exp1_Resnet_128_2048_v8"
    histogram_dir = "./histogram/exp1_Resnet_128_2048_v8"
