...
class SobolEstimator(ABC):
    ...
    def split_abc(outputs, nb_design, nb_dim):
        ...
        replication_c = np.array(
            [
                outputs[
                    nb_design * 2 + nb_design * i : nb_design * 2 + nb_design * (i + 1)
                ]
...
class HommaEstimator(SobolEstimator):
    ...
    def __call__(self, masks, outputs, nb_design):
        ...
        stis = [
            (
                var
                - (1.0 / nb_design) * np.sum(sampling_a * replication_c[i])
                + mu_a**2.0
            )
            / var
            for i in range(nb_dim)
...
class JanonEstimator(SobolEstimator):
    ...
    def __call__(self, masks, outputs, nb_design):
        ...
        mu_ac = [
            (1.0 / nb_design) * np.sum(sampling_a + replication_c[i]) / 2.0
            for i in range(nb_dim)
        ]
        var = [
            (1.0 / (nb_design - 1.0))
            * np.sum(sampling_a**2.0 + replication_c[i] ** 2.0)
            / 2.0
            - mu_ac[i] ** 2.0
            for i in range(nb_dim)
        ]
    ...
    def __call__(self, masks, outputs, nb_design):
        ...
        stis = [
            ...
            - (
                1.0
                / (nb_design - 1.0)
                * np.sum((sampling_a - mu_a) * (replication_c[i] - mu_c[i]))
                / (var_a * var_c[i]) ** 0.5
            )
            for i in range(nb_dim)
        ]
...
class SaltelliEstimator(SobolEstimator):
    ...
    def __call__(self, masks, outputs, nb_design):
        ...
        stis = [
            1.0
            - ((1.0 / nb_design) * np.sum(sampling_a * replication_c[i]) - mu_a**2.0)
            / var
            for i in range(nb_dim)
        ]
