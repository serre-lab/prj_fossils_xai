...
class Craft(BaseConceptExtractor):
    ...
    def fit(self, inputs: np.ndarray):
        ...
        assert (
            len(inputs.shape) == 4
        ), "Input data must be of shape (n_samples, channels, height, width)."
    ...
    def transform(self, inputs: np.ndarray, activations: Optional[np.ndarray] = None):
        ...
        if activations is None:
            activations = _batch_inference(
                self.input_to_latent, inputs, self.batch_size, device=self.device
            )
    ...
    def estimate_importance(self, inputs, class_id, nb_design=32):
        ...
        masks = HaltonSequence()(self.number_of_concepts, nb_design=nb_design).astype(
            np.float32
        )
        ...
        if len(U.shape) == 2:
            ...
            for u in U:
                ...
                stis = estimator(
                    torch_to_numpy(masks), torch_to_numpy(y_pred), nb_design
                )
