...
def nmf_value_and_gradcam(
    nmf, W, feature_model, x, concept_index, factoriseur, kkt_sol
):
    x = tf.constant(x)
    x = x[None, :, :, :]
    with tf.GradientTape(persistent=True) as tape:  # persistent important !!
        ...
        nnls_sol, backward_fun, state = jax.vjp(
            nmf.run, nnls_sol, (jnp.array(A), jnp.array(W)), has_aux=True
        )
        ...
        with tape.stop_recording():  # avoid the tape to record itself
            ...
            cotangeant_vector_nmf = base.KKTSolution(
                primal=cotangeant_primal, dual_eq=None, dual_ineq=cotangeant_dual
            )  # gradient wrt dual variables is zero.
            # the cotangeant vector is now a tangeant vector
            tangeant_vector_kkt_sol, (
                tangeant_vector_a,
                tangeant_vector_dict,
            ) = backward_fun(cotangeant_vector_nmf)
    ...
    gc_map = tf.reduce_sum(
        gcA * tf.reduce_mean(tangeant_vector_x, (1, 2))[:, None, None, :], -1
    )
    gc_map = tf.image.resize(
        tf.nn.relu(gc_map)[:, :, :, None], (224, 224), method="bicubic"
    )
    return gc_map, ui
...
def nmf_gradcam(feature_model, x, factoriseur, kkt_sol):
    pairs = [
        nmf_value_and_gradcam(feature_model, x, i, factoriseur, kkt_sol)
        for i in range(factoriseur.rank)
    ]
