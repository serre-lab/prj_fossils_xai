...
class NNLS(base.IterativeSolver):
    ...
    def __init__(
        ...
        conjugate_gradient_tolerance: float = 1e-6,
        init_mode: str = "bcd",
        implicit_diff: bool = True,
        implicit_diff_solver: Optional[Callable] = None,
    ...
    def init_optimality_function(self):
        ...
        return idf.make_kkt_optimality_fun(
            obj_fun=objective_function, eq_fun=None, ineq_fun=inequality_constraint
        )

    def init_params(self, params_obj: Tuple[jnp.array, jnp.array]):
        """
        ...
        kkt_solution = KKTSolution(primal=U, dual_eq=None, dual_ineq=U)
        return kkt_solution

    def init_state(self, init_params, params_obj: Tuple[jnp.array, jnp.array]):
        """
    ...
    def _compute_H_bar(self, H, G, F, U, rho, H_bar):
        ...
        H_bar, _ = jax.scipy.sparse.linalg.cg(
            matmul, right_member, x0=H_bar, tol=self.conjugate_gradient_tolerance
        )
        return H_bar
