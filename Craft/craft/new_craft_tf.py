...
class Craft(BaseConceptExtractor):
    ...
    def __init__(
        ...
        super().__init__(
            input_to_latent, latent_to_logit, number_of_concepts, batch_size
        )
    ...
    def fit2(self, inputs: np.ndarray, labels: np.ndarray):
        ...
        return activations, patches, labels

    def activation_transform(self, activations, patches):
    ...
    def transform(self, inputs: np.ndarray, activations: np.ndarray = None):
        ...
        activations = self.input_to_latent.predict(
            inputs, batch_size=self.batch_size, verbose=False
        )
    ...
    def estimate_importance(self, inputs, class_labels, nb_design=32):
        """
        ...
        if len(U.shape) == 2:
            ...
            for u in U:
                ...
                y_pred = self.latent_to_logit.predict(
                    a_perturbated, batch_size=self.batch_size, verbose=False
                )
    ...
    def new_estimate_importance(self, activations, class_labels, nb_design=32):
        ...
        U = self.new_transform(activations)
