...
def segmentation_sam(batch_input, batch_labels, batch_domain, batch_size):
    ...
    for x, y, d in zip(X, batch_labels, batch_domain):
        ...
        # cv2.imwrite(f'./sam_masks_new_4/image_{start}.png', sample_c)
        label = tf.repeat([tf.math.argmax(y)], 3, axis=0)
        samples.append(sample)
        labels.append(label)
        domains.append(tf.repeat([d], 3, axis=0))
        start += 1
    return (
...
if __name__ == "__main__":
    ...
    save_crops = "./crops/fossils_leaves_crops/exp5_RELU_192_20_v7"
    histogram_dir = "./histogram/exp5_RELU_192_20_v7"
