...
def segmentation_sam(batch_input, batch_labels, batch_domain, batch_size):
    ...
    labels = []
    domains = []
    start = batch_size * len(batch_input)
    for x, y, d in zip(X, batch_labels, batch_domain):
        ...
        mask_sum = mask.sum()
        if d == 1:
            if mask_sum < total_mask * 0.7:
                mask = torch.ones_like(mask)
        else:
            if mask_sum < total_mask * 0.15:
                mask = torch.ones_like(mask)
        sample = pi(x, mask)
        sample_c = np.hstack(sample)
        sample_c = np.hstack([x, sample_c])
        sample_c = cv2.cvtColor(sample_c, cv2.COLOR_RGB2BGR)
        cv2.imwrite(f"./sam_masks_new_6/image_{start}.png", sample_c)
        reps = sample.shape[0]
...
def evaluate(model, test_ds, batch_size=10, prob_augmentation=0.9, val_ds=None):
    ...
    for b, (batch_inputs, batch_labels, batch_domain) in enumerate(test_ds):
        ...
        input_images = tf.reshape(images, (-1, 384, 384, 3))
        labels = labels.numpy()
        ...
        print(f"Total correct images in batch {b} = {len(final_images)}")
        if len(final_images) > 0:
            ...
            for i in range(final_images.shape[0]):
                cv2.imwrite(
                    f"./patches_v4/image_{i}.jpg", final_images[i].numpy() * 255
                )

            if b % 5 == 0:
                if b > 0:
                    activations_and_patches = np.load(
                        "./activations/activations_patches_v2_64.npz"
                    )
                ...
                helpers.save_new_crops(
                    most_important_concepts, importances, crops_u, crops, save_crops, b
                )
        else:
            print(f"No correctly classified in batch {b}")


if __name__ == "__main__":
    model_path = "/cifs/data/tserre_lrs/projects/prj_fossils/models/Florissant_BEIT_xdomain_triplet_control_2024_florissant_k_leaves_fossils_xval_384_segmentation_split_0_thresh_25_propAug_0.7_TRIPLET_0.686_RELU_schedule_dilated_v2/model-11.h5"
    ...
    save_crops = "./crops/fossils_leaves_crops/exp5_RELU_192_20_v4"
    histogram_dir = "./histogram/exp5_RELU_192_20_v4"
